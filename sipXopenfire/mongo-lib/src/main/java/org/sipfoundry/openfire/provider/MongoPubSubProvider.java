...
public class MongoPubSubProvider extends BasePubSubProvider {
    ...
    public void createNode(PubSubService service, Node node) {
        DBObject toInsert = nodeToDBObject(service, node);

        getDefaultCollection().insert(toInsert);

        saveAssociatedElements(node, service);
    }
    ...
    public void updateNode(PubSubService service, Node node) {
        ...
        DBObject update = nodeToDBObject(service, node);

        getDefaultCollection().update(query, update);

        DBObject toDelete = new BasicDBObject();
        toDelete.put("serviceId", service.getServiceID());
    ...
    @Override
    public void saveAffiliation(PubSubService service, Node node, NodeAffiliate affiliate, boolean create) {
        if (create) {
            DBObject toInsert = new BasicDBObject();

            toInsert.put("serviceId", service.getServiceID());
    ...
    @Override
    public void removeAffiliation(PubSubService service, Node node, NodeAffiliate affiliate) {
        DBObject toDelete = new BasicDBObject();
    ...
    @Override
    public void saveSubscription(PubSubService service, Node node, NodeSubscription subscription, boolean create) {
        if (create) {
            DBObject toInsert = subscriptionToDBObject(service, node, subscription);
            ...
            if (NodeSubscription.State.none == subscription.getState()) {
                removeSubscription(service, node, subscription);
            } else {
                DBObject query = new BasicDBObject();
                ...
                DBObject update = subscriptionToDBObject(service, node, subscription);
    ...
    @Override
    public void removeSubscription(PubSubService service, Node node, NodeSubscription subscription) {
        DBObject toDelete = new BasicDBObject();
    ...
    public void loadSubscription(PubSubService service, Node node, String subId) {
        ...
        if (subscriptionObj != null) {
            loadSubscriptions(service, nodes, subscriptionObj);
        }
    }
    ...
    private static void saveAssociatedElements(Node node, PubSubService service) {
        DBCollection nodeJidCollection = getCollection(NODE_JID_COLLECTION_NAME);

        for (JID jid : node.getContacts()) {
            DBObject toInsert = nodeJidToDBObject(node, service, jid, "contacts");

            nodeJidCollection.insert(toInsert);
        }
        for (JID jid : node.getReplyRooms()) {
            DBObject toInsert = nodeJidToDBObject(node, service, jid, "replyRooms");

            nodeJidCollection.insert(toInsert);
        }
        for (JID jid : node.getReplyTo()) {
            DBObject toInsertContact = nodeJidToDBObject(node, service, jid, "replyTo");

            nodeJidCollection.insert(toInsertContact);
        }
        if (node.isCollectionNode()) {
            for (JID jid : ((CollectionNode) node).getAssociationTrusted()) {
                DBObject toInsert = nodeJidToDBObject(node, service, jid, "associationTrusted");
    ...
    private static DBObject nodeToDBObject(PubSubService service, Node node) {
        DBObject nodeObj = new BasicDBObject();

        nodeObj.put("serviceId", service.getServiceID());
    ...
    private static DBObject nodeJidToDBObject(Node node, PubSubService service, JID jid, String type) {
        DBObject nodeObj = new BasicDBObject();
    ...
    private static DBObject subscriptionToDBObject(PubSubService service, Node node, NodeSubscription subscription) {
        DBObject subscrObj = new BasicDBObject();

        subscrObj.put("serviceId", service.getServiceID());
    ...
    private static void loadSubscriptions(PubSubService service, Map<String, Node> nodes, DBObject subscriptionObj) {
        ...
        if (node != null) {
            ...
            if (node.getAffiliate(owner) != null) {
                ...
                NodeSubscription subscription = new NodeSubscription(service, node, owner, subscriber,
