...
public class JobFactory {
    ...
    public static Job createJob(MongoOperation op, DBObject dbObj, Object id) {
        logger.debug("Creating job for " + id);
        Job job = null;
        ...
        if (id != null) {
            if (id instanceof String) {
                ...
                logger.debug("Creating vcard update job");
    ...
    private static Job createJob(MongoOperation op, DBObject dbObj, Long id) {
        Job job = null;

        // exercise care when adding other entities to watch. additional discriminators will
        // be needed
        Long lid = null;
        try {
            lid = Long.valueOf(id);
        } catch (NumberFormatException nfe) {
            logger.warn(String.format("Could not parse id from %s", id));
        }
    ...
    private static Job createJob(MongoOperation op, DBObject dbObj, ObjectId id) {
        ...
        case INSERT:
            ...
            break;
        case DELETE:
            String imid = CacheHolder.getAvatar(id.toString());
            logger.debug("delete for " + imid);
            vcardUpdateJob = imid != null ? new VcardUpdateJob(imid, null) : null;
            break;
        }
